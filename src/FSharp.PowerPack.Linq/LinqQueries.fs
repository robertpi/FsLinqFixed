...
module Query =
    ...
    let MakeQueryableSkip = 
        let F = MakeGenericStaticMethod <@ System.Linq.Queryable.Skip @> 
        fun (srcTy,src,count)  -> 
            let src = Expr.Coerce(src,MakeIQueryableTy srcTy)
            F ([srcTy],[src;count])
    ...
    let (|PipedCallSeqSkip|_|) = (|SpecificPipedCall1|_|) <@ Seq.skip @>
    ...
    let query (p : Expr<'T1>) : 'T1 = 
        ...
        let rec TransInner (tm:Expr) = 
            ...
            | PipedCallSeqSkip([ srcTy ],count,sq) ->
                MakeQueryableSkip(srcTy, TransInner sq, MacroExpand count)
